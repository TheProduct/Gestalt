...
public class PointSprite
    ...
    private static boolean _myIsInitStatic = false;

    private static float _myMaxSupportedSize;

    public float minpointsize = 1;

    public float maxpointsize = 128;

    public float pointsize = 1;

    public float fadethreshold = 1;

    public static final int ATTENUATION_CONSTANT = 0;

    public static final int ATTENUATION_LINEAR = 1;

    public static final int ATTENUATION_QUAD = 2;

    public float[] quadric = new float[] {10,
                                          0.002f,
                                          0.000001f};
    ...
    public PointSprite(final boolean theHintFlipYAxis) {
    ...
    public void begin(GLContext theRenderContext, Material theParent) {
        ...
        if (!_myIsInitStatic) {
            _myIsInitStatic = true;

            /* test extension */
            if (!gl.isExtensionAvailable("GL_ARB_point_sprite")) {
                System.err.println("### ERROR @ PointSprite / GL_ARB_point_sprite not available.");
            }

            /* query for the max point size supported by the hardware */
            gl.glEnable(GL.GL_POINT_SPRITE_ARB);
            float[] maxSizeArray = new float[1];
            gl.glGetFloatv(GL.GL_POINT_SIZE_MAX_ARB, maxSizeArray, 0);
            _myMaxSupportedSize = maxSizeArray[0];
        }
        ...
        if (quadric != null && quadric.length == 3) {
             ...
             * float pointsize = 500;
             * float d = 300;
             * float myPointSize = pointsize * (float) Math.sqrt(1 / (a + b * d + c * d * d));
             * }
             */


            gl.glPointParameterfvARB(GL.GL_POINT_DISTANCE_ATTENUATION_ARB, quadric, 0);
        }
         ...
         */
        gl.glPointParameterfARB(GL.GL_POINT_FADE_THRESHOLD_SIZE_ARB, fadethreshold);
...
//        gl.glTexEnvi(GL.GL_POINT_SPRITE_ARB, GL.GL_COORD_REPLACE_ARB, GL.GL_FALSE);
        ...
        /* set origin to work well with java bitmaps */
    ...
    public static float getMaxSupportedPointSize() {
        if (!_myIsInitStatic) {
            System.err.println(
                    "### WARNING @ JoglPointSprite / value is not initalized before the first draw. come back later.");
        }
        return _myMaxSupportedSize;
    }
}
